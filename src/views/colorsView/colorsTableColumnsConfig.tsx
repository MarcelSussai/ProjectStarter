...
export const ColorsTableColumnsConfig: I.ICollunnsConfig<IRowData>[] = [
  {
    idKey: 'nameColor', name: 'Cor', order: 2,
    ValueComponent: ({row}) => (
      <ColorBox color={row.tones.c500 || 'transparent'}>
        <S.WrapperNameColor>
          <div> {row.nameColor} </div>
          <div> {`500`} </div>
        </S.WrapperNameColor>
      </ColorBox>
    ),
    sizeOfColumn: { min: '200px', max: '200px' },
    hideByDefault: false,
  },
  {
    idKey: 'varColor', name: 'Exemplo de variável CSS', order: 3,
